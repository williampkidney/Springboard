...
SELECT bk.member, bk.facility, bk.cost 
FROM (SELECT CONCAT_WS(" ", m.firstname, m.surname) as member, f.name AS facility,
		...
		WHERE b.starttime LIKE '2012-09-14%') AS bk
WHERE bk.cost > 30
ORDER BY bk.cost DESC;
...
SELECT f.name, COUNT(sub.memid) AS facility_member_usage
FROM
	(SELECT facid, memid
     FROM Bookings
...
SELECT sub.month, COUNT(sub.memid) AS facility_member_monthly_usage
FROM 
